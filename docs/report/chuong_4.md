...
#### 4.2.3.4. Nhóm thanh toán

`Payment` phản ánh tiến trình thanh toán liên kết chặt với `SalesOrder`, cho phép hệ thống xử lý linh hoạt nhiều phương thức thanh toán và các trạng thái trung gian (khởi tạo, thành công, thất bại, hết hạn). Việc lưu `transactionRef` hỗ trợ đối soát ngoài hệ thống, phục vụ kiểm toán và xử lý sai lệch giao dịch.
...
5. Các giá trị tính toán theo giao dịch (tổng tiền, chiết khấu) được lưu tại đơn hàng để đảm bảo truy vết lịch sử.
...
| product_variants | weight | DECIMAL(10,3) | DEFAULT 0 | Thông số trọng lượng phục vụ phân tích thống kê |
...
| orders | service_fee | DECIMAL(12,2) | NOT NULL, DEFAULT 0 | Phụ phí dịch vụ |
...
*Bảng 4.11: Cấu trúc dữ liệu bảng `reviews`.*
...
Tất cả bảng nghiệp vụ cốt lõi đều sử dụng khóa chính độc lập để định danh bản ghi. Các khóa ngoại được thiết kế theo quan hệ nghiệp vụ chặt chẽ, tránh phát sinh bản ghi mồ côi, đặc biệt tại các bảng giao dịch như `order_items` và `payments`. Với các quan hệ có tính bắt buộc, ràng buộc `NOT NULL` được áp dụng để loại trừ trạng thái dữ liệu không hợp lệ.

### 4.6.2. Toàn vẹn miền giá trị

Các thuộc tính trạng thái (đơn hàng, thanh toán, tài khoản) được giới hạn trong tập giá trị xác định trước. Các cột tiền tệ dùng kiểu `DECIMAL(12,2)` để bảo đảm chính xác số học tài chính. Thuộc tính đánh giá (`rating`) giới hạn theo thang điểm chuẩn, qua đó nâng cao độ tin cậy dữ liệu phân tích chất lượng dịch vụ.

### 4.6.3. Chỉ mục và tối ưu truy vấn nghiệp vụ

Để hỗ trợ truy vấn thường xuyên, cần ưu tiên tạo chỉ mục cho các trường: `email`, `order_code`, `transaction_ref`, `product_id`, `category_id`, `status`, `ordered_at`. Cách tiếp cận này cải thiện hiệu năng cho các tác vụ tìm kiếm sản phẩm, tra cứu đơn hàng, đối soát thanh toán và báo cáo vận hành theo thời gian.
...
stateDiagram-v2
    ...
    PACKING --> DELIVERED : Hoan tat phuc vu dat phong
    PACKING --> RETURNED : Huy/hoan sau khi xac nhan
    DELIVERED --> [*]
    CANCELLED --> [*]
    RETURNED --> [*]
```
